...
import io
import base64
from cyberhunter_3d.web.models import db, User, Scan, Target, Vulnerability
...
def register_routes(app, bcrypt):
    ...
    def submit_targets():
        ...
        db.session.commit()
        current_app.executor.submit(run_discovery_phase, new_scan.id, current_app._get_current_object())
    ...
    def sync_hackerone():
        ...
        current_app.executor.submit(check_for_new_targets, current_app._get_current_object())
        return redirect(url_for('dashboard'))
