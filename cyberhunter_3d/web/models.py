...
class Asset(db.Model):
    ...
    first_seen = db.Column(db.DateTime, nullable=False, default=now_utc)
    last_seen = db.Column(db.DateTime, nullable=False, default=now_utc, onupdate=now_utc)
    scan_id = db.Column(db.Integer, db.ForeignKey('scan.id'), nullable=False)
...
class Schedule(db.Model):
    """
    Schedule model for running scans at regular intervals.
    """
    id = db.Column(db.Integer, primary_key=True)
    target_id = db.Column(db.Integer, db.ForeignKey('target.id'), nullable=False, unique=True)
    target = db.relationship('Target', backref=db.backref('schedule', uselist=False))
    ...
    def __repr__(self):
        return f'<Schedule for Target {self.target_id} - {self.frequency}>'

>>>>>>> feat/initial-recon-module
