...
class TestPluginManager(unittest.TestCase):
    ...
    def test_dependency_resolution_simple(self):
        """
        Tests that a simple, valid dependency chain is resolved correctly.
        """
        ...
        p3.requires = ["B"]
        p3.provides = ["C"]
        plugin_manager.plugins = [p3, p2, p1]  # Unordered
    ...
    def test_dependency_resolution_missing(self):
        """
        Tests that the resolver raises an exception for a missing dependency.
        """
        with patch('cyberhunter_3d.core.plugins.manager.PluginManager._discover_plugins', return_value=[]):
            plugin_manager = PluginManager()
    ...
    def test_dependency_resolution_circular(self):
        """
        Tests that the resolver raises an exception for a circular dependency.
        """
        with patch('cyberhunter_3d.core.plugins.manager.PluginManager._discover_plugins', return_value=[]):
            plugin_manager = PluginManager()
    ...
    def test_run_all_plugins_with_dependencies(self):
        """
        Tests that the plugin manager runs plugins in the correct, dependency-sorted order.
        """
        ...
        plugin_manager = PluginManager()
        plugin_manager.plugins = [plugin3, plugin1, plugin2]
    ...
    def test_discover_plugins_mixed(self):
        """
        ...
        new_plugin = next((p for p in plugin_manager.plugins if p.name == "new_style_test_plugin"), None)
        old_plugin_wrapper = next((p for p in plugin_manager.plugins if p.name == "old_style_test_plugin"), None)

        self.assertIsNotNone(new_plugin)
        self.assertIsNotNone(old_plugin_wrapper)

        from cyberhunter_3d.core.plugins.manager import OldPluginWrapper
        self.assertIsInstance(old_plugin_wrapper, OldPluginWrapper)
