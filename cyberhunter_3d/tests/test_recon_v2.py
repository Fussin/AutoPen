...
def test_full_recon_pipeline_mocked(
    ...
    tech_results = {'http://active.example.com': {'technologies': ['nginx']}}
    ...
    expected_calls = [
        call(list(all_final_domains), "subdomains_verified.json", ANY),
        call(mock_ip_map, "subdomain_ip_mapping.json", ANY),
        call(mock_asn_details, "asn_details.json", ANY),
        call(list(live_hosts), "live_hosts.json", ANY),
        call({'http://active.example.com': ['nginx']}, "tech_fingerprinting.json", ANY),
        call(takeover_findings, "takeover_vulnerabilities.json", ANY),
        call(list(code_subs), "code_analysis_subdomains.json", ANY),
        call(cloud_assets, "cloud_assets.json", ANY)
    ]
    # We don't check call count because some files might be empty and not saved
