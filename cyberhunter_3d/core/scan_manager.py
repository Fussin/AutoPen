...
def run_discovery_phase(scan_id, app):
    ...
    with app.app_context():
        ...
        @analytics.track_phase_duration('discovery')
        def discovery_work():
            ...
            try:
                ...
                analytics.increment_metric('discovery.subdomains.total', len(subdomains))
                dns_records = {}
                alive_subdomains = []
                dead_subdomains = []
                for s in subdomains:
                ...
                analytics.increment_metric('discovery.subdomains.alive', len(alive_subdomains))
                analytics.increment_metric('discovery.subdomains.dead', len(dead_subdomains))
                ...
                analytics.increment_metric('discovery.assets.in_scope', in_scope_count)
                analytics.increment_metric('discovery.assets.out_of_scope', out_of_scope_count)
    ...
    with app.app_context():
        ...
        def execution_work():
            ...
            try:
                ...
                total_ips = Asset.query.filter(Asset.scan_id == scan.id, Asset.type.in_(['ip_address', 'cidr'])).count()
                analytics.update_scan_coverage(total_ips, len(ip_targets))
                ...
                for target in ip_targets:
                    ...
                    analytics.increment_metric('discovery.ports.scanned_hosts')
                    for asset_data in ip_scan_assets:
                        ...
                        analytics.track_vulnerability_severity("informational")
                        vuln_id_counter += 1
                ...
                analytics.increment_metric('discovery.rdns.found', rdns_found_count)
                ...
                if unique_domains:
                    ...
                    for domain in related_domains:
                        if validator.is_in_scope(domain) and not Asset.query.filter_by(scan_id=scan.id, value=domain).first():
                            ...
                            out_of_scope_count += 1
                analytics.increment_metric('discovery.analytics_correlation.found', analytics_found_count)
